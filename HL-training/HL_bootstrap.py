...
def run_bootstraps(rinv):
    ...
    for train_index, test_index in t:
        ...
        if not model_file.exists():
            ...
            callbacks = [keras.callbacks.EarlyStopping(patience=10, verbose=0, restore_best_weights=True),
                         keras.callbacks.ModelCheckpoint(filepath=model_file, verbose=0, save_best_only=True)]
        ...
        bs_count += 1
        counter += 1     
        t.set_description(f"Run ({counter}/{n_splits}): (AUC = {auc_mean:.5f} +/- {auc_std:.5f}")
        t.refresh()
