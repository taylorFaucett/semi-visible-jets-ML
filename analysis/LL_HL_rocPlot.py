...
for bkgRej_log in (True, False):
    ...
    for rix, rinv in enumerate(rinvs):
        for data_type in data_types:
            auc_file = (
                path / f"{data_type}_training" / "bootstrap_results" / rinv / "aucs.csv"
            )
            if auc_file.exists():
                aucs = pd.read_csv(auc_file).auc.values
                auc_avg, auc_ci = np.mean(aucs), np.percentile(aucs, (2.5, 97.5))
                auc_ci_max = max(
                    np.abs(auc_ci[0] - auc_avg), np.abs(auc_ci[1] - auc_avg)
                )
                roc_path = (
                    path / f"{data_type}_training" / "bootstrap_results" / rinv / "roc"
                )
                for ix, (roc_file, auc) in enumerate(
                    zip(list(roc_path.glob("*")), aucs)
                ):
                    ...
                    if data_type == "HL":
                        ...
                        ls = "-"
                    label = "%s [$\mathrm{AUC} = %0.3f \pm %0.3f$]" % (
                        data_type,
                        auc_avg,
                        auc_ci_max,
                    )
                    ...
                    if bkgRej_log:
                        ...
                        axs[rix].plot(
                            [1, 0], [0, 1], lw=2, color="k", linestyle="dotted"
                        )
                        axs[rix].plot(
                            roc_df.sig_eff,
                            1.0 - roc_df.bkg_eff,
