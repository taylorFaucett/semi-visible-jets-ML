...
svj_path = "/DFS-L/DATA/atlas/tfaucett/semi-visible-jets-ml"
data_path = svj_path + "/data"
generation_path = svj_path + "/generation"
gen_path = generation_path + "/gen"
...
def swap_pdg_id(run_name):
    lhe_path = data_path + "/raw_data/sig_schannel/%s/%s/Events/run_01" % (
        rinv_label,
        run_name,
    )
...
def gen_lhe(run_name):
    ...
    rinv_dir = data_path + "/raw_data/sig_schannel/%s" % (rinv_label)
    run_dir = "%s/%s" % (rinv_dir, run_name)
    ...
    process_file = "%s/lhe_proc/sig_schannel_%s.txt" % (proc_path, run_name)
    dict_replace(mg5_template, process_file, d)
    log_file = log_path + "/mg5/%s-%s.log" % (rinv_label, run_name)
...
def pythia8_delphes(run_name):
    ...
    out_path = (
        data_path
        + "/raw_data/sig_schannel/%s/%s/Events/run_01/unweighted_events.lhe"
        % (rinv_label, run_name)
    )
